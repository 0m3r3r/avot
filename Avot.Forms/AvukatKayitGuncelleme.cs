...
{
    public partial class AvukatKayitGuncelleme : Form
    {
        public AvukatKayitGuncelleme()
        {
            InitializeComponent();
        }

        int ID;
        public AvukatKayitGuncelleme(int GuncellenecekItemID)
        {
            InitializeComponent();
        ...
        AdresService adrService = new AdresService();
        ...
        {
            if (!(string.IsNullOrEmpty(mtxtTC.Text)))
            {
                adrService.Insert(new AdresDTO()
                   {
                       ...
                       KayitTarihi = Convert.ToDateTime(DateTime.Now.ToLongDateString()),
                       KaydiDegistirmeTarihi = Convert.ToDateTime(DateTime.Now.ToLongTimeString()),
                   });
                ...
                nService.Insert(new NufusDetayDTO()
                {
                    ...
                    ILID = (int)CmbKimlikilce.SelectedValue,
                ...
                aService.Insert(new AvukatDTO()
                {

                    TC = mtxtTC.Text,
                    Adi = txtAd.Text,
                    Soyadi = txtSoyad.Text,
                    Brans = txtBransi.Text,
                    BaroSicilNo = txtBaroSicilNo.Text,
                    TBBNo = txtTBBNo.Text,
                    VergiDairesi = txtVergiDairesi.Text,
                    VergiNo = txtVergiNo.Text,
                    Cinsiyet = cmbCinsiyet.Text == "Erkek" ? true : false,
                    CalisiyorMu = chckCalisiyorMu.Checked == true ? true : false,
                    KaydiGirenKullaniciID = User.ID(),
                    KaydiDegistirenKullaniciID = User.ID(),
                    KayitTarihi = DateTime.Now,
                    KaydiDegistirmeTarihi = DateTime.Now,
            ...
            {
                MessageBox.Show("Tc alanını eksiksiz doldurunuz.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        private void dataUpdate()
        {
            try
            {
                aService.Select().Where(p => p.ID == ID);

                aService.Update(new AvukatDTO()
                {
                    TC = mtxtTC.Text,
                    Adi = txtAd.Text,
                    Soyadi = txtSoyad.Text,
                ...
                nService.Insert(new NufusDetayDTO()
                {
                    ...
                    Dini = txtDini.Text,
                    KanGrubu = txtKanGrubu.Text,
                    ILID = Convert.ToInt32(CmbKimlikil.SelectedValue),
                    Mahalle = txtMahalle.Text,
                    Koy = txtKoy.Text,
                    CiltNo = txtCiltNo.Text,
                    AileSiraNo = txtAileSiraNo.Text,
                ...
                {
                    AdresBaglantiliKisiTC = mtxtTC.Text,//kişi TC sini alıp tablo içerisindeki identity PK kolonuna basıyor.
                    //ILID = Convert.ToInt32(cmbAdresililce.SelectedValue),
                    Tel1 = txtTel1.Text,
                    Tel2 = txtTel2.Text,
                    Tel3 = txtTel3.Text,
                    Fax = txtFax.Text,
        ...
        private void AvukatKayitGuncelleme_Load(object sender, EventArgs e)
        {
            label41.Text = ID.ToString();

            if (!(ID <= 0))
            {
                foreach (AvukatDTO item in aService.Select().Where(p => p.ID == ID))
                {//Üst bölümde listviewitem in tag inden gelen id yi ID değişkeninin içerisine aldık.Burada ise foreach ile avukatservice ye select çekiyoruz ve where koşulu koyarak bana şu id ye ait avukatın bilgilerini getir diyoruz.Gelen bilgiler item içerisinde, ekrana basma işlemini gerçekleştiriyoruz.

                    //Genel Bilgiler Bölümü
                    txtAd.Text = item.Adi;
                    txtSoyad.Text = item.Soyadi;
                    mtxtTC.Text = item.TC;
                    txtBransi.Text = item.Brans;
                    txtBaroSicilNo.Text = item.BaroSicilNo;
                    txtTBBNo.Text = item.TBBNo.ToString();
                    txtVergiDairesi.Text = item.VergiDairesi;
                    txtVergiNo.Text = item.VergiNo.ToString();
                    cmbCinsiyet.Text = item.Cinsiyet == true ? "Erkek" : "Kadın";
                    chckCalisiyorMu.Checked = item.CalisiyorMu == true ? true : false;
                }
                ...
                foreach (AdresDTO item in adrService.Select().Where(p => p.AdresBaglantiliKisiTC == mtxtTC.Text))
                {
                    CmbKimlikil.SelectedValue = item.ILID;
            ...
            //CmbAdresil.DataSource = iiService.Select();
            //CmbAdresil.DisplayMember = "Il";
            //CmbAdresil.ValueMember = "IlKodu";

            //CmbKimlikilce.DataSource = iiService.Select();
            //CmbKimlikilce.DisplayMember = "Ilce";
            //CmbKimlikilce.ValueMember = "IlceKodu";
        ...
        private void btnKaydetAlt_Click(object sender, EventArgs e)
        {
            if (ID <= 0)
            {
                dataSave();
